	...
	public acquireToken(callback: OnTokenAcquiredCallback) {
		(async() => {
			...
			try {
				...
				vscode.window.showErrorMessage(`Could not login: ${error}`);
				return;
			}
					...
					this.adalContext.acquireTokenWithDeviceCode(
						...
						(error: Error, response: TokenResponse | ErrorResponse) => {
							if (error) {
								...
								if (!tokenResponse) {
									reject('[AUTH] Could not acquire access token with device code flow - Error Response: ' + response.error + ' Desc: ' + response.errorDescription);
								} else {
									console.log('[AUTH] Received access token with device code flow. Valid until: ' + tokenResponse.expiresOn);
	...
	private tryGetAccessTokenWithRefreshToken(): Promise<string | undefined> {
		return new Promise((resolve, reject) => {
			if (!this.refreshToken) {
				...
				this.adalContext.acquireTokenWithRefreshToken(this.refreshToken, this.aadClientId, this.resourceUrl, (error: Error, response: TokenResponse | ErrorResponse) => {
					if (error) {
						reject('[AUTH] Could not acquire access token with refresh token: ' + error.message);
					} else {
						const tokenResponse = <TokenResponse>response;
						if (!tokenResponse) {
							reject('[AUTH] Could not acquire access token with refresh token - Error Response: ' + response.error + ' Desc: ' + response.errorDescription);
						} else {

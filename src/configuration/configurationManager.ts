...
export class ConfigurationManager {
    ...
    get isConfigured(): boolean {
        if (this.authenticationMethod && this.d365InstanceName && this.d365CrmRegion && this.credentialManager?.isConfigured) {
            return true;
        }
        return false;
    }
    ...
    public async askForPortalDataFileMigration(): Promise<boolean> {
        ...
        if (pickedOption === options[0]) {
            await this.migrateToFolderFileStructure();
            return true;
        }
    ...
    private async configure(context: ExtensionContext) {
        ...
        if (!config) {
            return await DialogReporter.reportError("Could not get configuration for Portal connection. Please try again.");
        }
        ...
        this.credentialManager = new CredentialManager(config.aadTenantId, config.aadClientId, this.authenticationMethod);

        if (this.authenticationMethod === AuthenticationMethod.clientCredentials) {
            await this.credentialManager.setSecret(config.aadClientSecret);
        }
    ...
    private async storeConfiguration(): Promise<void> {
        ...
        await workspace.getConfiguration().update(PORTAL_SETTING_PREFIX_ID + "." + SETTINGS_TENANT_ID, this.credentialManager.aadTenantId);
        await workspace.getConfiguration().update(PORTAL_SETTING_PREFIX_ID + "." + SETTINGS_CLIENT_ID, this.credentialManager.aadClientId);

        await workspace.getConfiguration().update(PORTAL_SETTING_PREFIX_ID + "." + SETTINGS_INSTANCE_NAME, this.d365InstanceName);
        ...
        await workspace.getConfiguration().update(PORTAL_SETTING_PREFIX_ID + "." + SETTINGS_FOLDER_FILES, this.useFoldersForWebFiles);

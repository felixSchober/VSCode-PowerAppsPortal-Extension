...
export class PortalData {
    ...
    public getDocumentContent(uri: Uri, decodeFromBase64: boolean = true): string {
        const fileType = getFileType(uri);
        ...
        switch (fileType) {
            ...
            case PortalFileType.webFile:
                return decodeFromBase64 ? this.getDecodedWebFileContents(uri) : this.getBase64WebFileContents(uri);
            default:
                return "";
        }
    }

    private getBase64WebFileContents(uri: Uri): string {
        return this.getWebFile(uri)?.b64Content || "";
    }

    private getDecodedWebFileContents(uri: Uri): string {
        const content = this.getBase64WebFileContents(uri);

        const decodingCharacterSet = this.getDecodingCharacterSet(content);
        let decodedContent = Buffer.from(content, BASE64).toString(decodingCharacterSet);
        decodedContent = this.removeByteOrderMark(decodedContent);
        return decodedContent;
    }

    /**
     * Checks if a `base64` encoded string has a little endian byte order mark (BOM) at the start.
     * @param str Base64 encoded string to check
     * @returns  `true` if the string has a BOM, `false` otherwise
     */
    private hasLEBOM(str: string): boolean {
        const littleEndianBOM = "//4";
        return str.startsWith(littleEndianBOM);
    }

    private getDecodingCharacterSet(base64String: string): BufferEncoding {
        if (this.hasLEBOM(base64String)) {
            return "utf16le";
        }
    ...
    private removeByteOrderMark(content: string): string {
